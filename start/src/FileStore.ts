...
//     return path.join(__dirname, this.directory, `${id}.txt`)      //Adding to a new file PathResolver
//   }
// }
...
import { FileWriter, FileReader } from "./FileService";
...
export class FileStore {
  ...
  async save(id: number, message: string) {
    console.log("Saving message: ", id);
    const filePath = new PathResolver(this.directory).getFilePath(id);

    await this.fileWriter.write(filePath, message);
    this.cacheManager.set(id, message);
  ...
  read(id: number): string {
    ...
    //should check the cache with .has then set using read()

    if (!this.cacheManager.has(id)) {
      console.info(`Message id ${id} not in cache`);
      const data = this.fileReader.read(filePath);
      this.cacheManager.set(id, data);
    }
  }

  checkCache(){
    this.cacheManager.checkCache()
  }
}
